...
import { Plus, Heart, Music, ListMusic, MoreHorizontal, Trash2, Play } from 'lucide-react';
        ...
        <div className="space-y-2">
          {likedSongs.map((song, idx) => (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">{song.duration_ms ? `${Math.floor(song.duration_ms / 60000)}:${String(Math.floor((song.duration_ms % 60000) / 1000)).padStart(2, '0')}` : '--:--'}</p>
            </motion.div>
          ))}
          {likedSongs.length === 0 && (
